...
def register_features_generator(features_generator_name: str) -> Callable[[FeaturesGenerator], FeaturesGenerator]:
    ...
    :param features_generator_name: name to use to access the features generator.
    :return: decorator which will add a features generator to the registry using the specified name.
    """
...
def get_features_generator(features_generator_name: str) -> FeaturesGenerator:
    ...
    :param features_generator_name: name of features generator
    :return: desired features generator
    """
...
def get_available_features_generators() -> List[str]:
    """ Returns a list of names of availabe features generators."""
...
def morgan_binary_features_generator(mol: Molecule,
    ...
    :param mol: a molecule (either SMILES or RDKit molecule)
    :param radius: Morgan fingerprint radius
    :param num_bits: Number of bits in Morgan fingerprint.
    :return: 1D numpy array containing the binary Morgan fingerprint
    """
...
def morgan_counts_features_generator(mol: Molecule,
    ...
    :param radius: Morgan fingerprint radius
    :param num_bits: number of bits in Morgan fingerprint
    :return: a 1D numpy array contianing counts-based Morgan fingerprint
    """
    mol = Chem.MolFromSmiles(mol) if type(mol) == str else mol
...
try:
    ...
    def rdkit_2d_features_generator(mol: Molecule) -> np.ndarray:
        ...
        :param mol: a molecule, either SMILES or RDKit molecule
        :return: a 1D numpy array with RDKit 2D features
        """
    ...
    def rdkit_2d_normalized_features_generator(mol: Molecule) -> np.ndarray:
        ...
        :param mol: a molecule, either SMILES or RDKit
        :return: 1D numpy array with RDKit 2D normalized features.
        """
        smiles = Chem.MolToSmiles(mol, isomericSmiles=True) if type(mol) != str else mol
    ...
    @register_features_generator('rdkit_2d')
    def rdkit_2d_features_generator(mol: Molecule) -> np.ndarray:
        """ Mock implementation raising Import error if descriptastorus cannot be imported"""
        raise ImportError('Failed to import descriptastorus, please install it to use RDKit 2D features.')
