...
def save_features(path: str, features: List[np.ndarray]) -> None:
    ...
    :param path: path to :code:`.npz` file with array name "features"
    :param features: list of 1D numpy arrays with features for molecules
    """
...
def load_features(path: str) -> np.ndarray:
    ...
    * :code:`.pkl` / :code:`.pckl` / :code:`.pickle` containing a sparse numpy array
    ...
    :param path: Path to file containing features
    :return: 2D numpy array of size :code:`(num_molecules, features_size)` containing the features.
    """
...
def load_valid_atom_or_bond_features(path: str, smiles: List[str]) -> List[np.ndarray]:
    """
    Loads features saved in variety of formats including:

    * :code:`.npz` descriptors saved as 2D array for each molecuel in order of that in data.csv
    ...
    * :code:`.sdf` containing all mol blocks with descriptors as entries
    :param path: path to file contianing atomwise features.
